...
void setup() {
  ...
  pinMode(6,INPUT_PULLUP);//DSC08刹车
...
void loop() {
  JPWM=map(analogRead(A2),0,1024,80,250);//获取DSC08的PWM信号
  CHWM=map(analogRead(A3),0,1024,80,250);//获取轴动小车的PWM信号
  analogWrite(9,CHWM);//轴向小车PWM输出
  analogWrite(5,CHWM);//DSC08PWM输出
